...
def intent_classifier(text):
    url = 'http://192.168.45.34:5002/service/api/bert'
    data = {"text":text}
    # print(data)
...
def slot_recognizer(text):
    url = 'http://192.168.45.34:5001/service/api/ner'
    data = {"text_list":text}
    headers = {'Content-Type':'application/json;charset=utf8'}
    reponse = requests.post(url,data=json.dumps(data),headers=headers)
    if reponse.status_code == 200:
...
def text_analysis(msg):
    ...
    # 确保 intent_msg 和 entity 都不是 -1，并且 intent_msg 是一个字典
    # print(intent_msg)
    # if isinstance(intent_msg, dict) and intent_msg.get("name") == '其他':
    #     return semantic_slot.get("unrecognized")
    # if intent_msg == -1 or entity == -1:
    #     return semantic_slot.get("unrecognized")
