...
class MedicalKG(object):
    def __init__(self):
        ...
        self.graph = Graph("http://localhost:7474", user = 'neo4j', password = '12345678', name = 'neo4j')
        # # 共8类节点
        self.drugs = []  # 药品
        self.recipes = []  # 菜谱
        self.foods = []  # 食物
    ...
    def Triplet(self,data_path):
        ...
        with open(data_path,"r",encoding="utf-8") as f:
            ...
            data = json.loads(f.read())
            for data_json in tqdm(data["RECORDS"],ncols=80):

                disease_dict = {}
                # disease=data_json['basic_info']['name']
                disease = eval(data_json['basic_info'])['name']
                ...
                if "并发症" in eval(data_json['basic_info']):
                    disease_acompany=eval(data_json['basic_info'])["并发症"].split(' ')
                ...
                if 'desc' in eval(data_json['basic_info']):
                    disease_dict['desc']=eval(data_json['basic_info'])["desc"]
                ...
                if "患病比例" in eval(data_json['basic_info']):
                    disease_dict["easy_get"]=eval(data_json['basic_info'])["患病比例"]
                if "易感人群" in eval(data_json['basic_info']):
                    disease_dict['easy_get']=eval(data_json['basic_info'])['易感人群']
                if '就诊科室' in eval(data_json['basic_info']):
                    department=eval(data_json['basic_info'])['就诊科室'].rstrip().split("  ")
                    # print(department)
                    if len(department)==1:
                ...
                if "传染方式" in eval(data_json['basic_info']):
                    disease_dict['cure_way']=eval(data_json['basic_info'])["传染方式"]
                if "治疗周期" in eval(data_json['basic_info']):
                    disease_dict['cure_lasttime']=eval(data_json['basic_info'])["治疗周期"]
                if "治愈率" in eval(data_json['basic_info']):
                    disease_dict['cured_prob']=eval(data_json['basic_info'])['治愈率']
                if " 常用药品" in eval(data_json['basic_info']):
                    common_drug = eval(data_json['basic_info'])[" 常用药品"].split(" ")
            ...
            # print(self.rels_commonddrug)
                if 'food_info' in data_json :
                    recommand_eat=eval(data_json['food_info'])['good_cooking_reconmand']
                    self.rels_recommandeat.append([disease,"recommand_eat",recommand_eat])
                    self.recipes.append(recommand_eat)
                    if "bad_eat" in eval(data_json['food_info']) :
                        not_eat=eval(data_json['food_info'])['bad_eat'].split(',')
                        for eat_not in not_eat:
                            self.rels_noteat.append([disease,"not_eat",eat_not])
                            self.foods.append(eat_not)
                    if 'good_eat' in eval(data_json['food_info']):
                        good_eat=eval(data_json['food_info'])['good_eat'].split(',')
                ...
                if "drug_info" in data_json:
                    for drug_det in eval(data_json['drug_info']):
            ...
            if e_dict["name"]:
                ...
                for key, value in e_dict.items():
                    ...
                    if key =='cure_department':
                        ...
                        # cql = f"MATCH (n:{etype}) WHERE n.name=$name SET n.{key}=$value"
                        # self.graph.run(cql, name=name, value=value)
                    ...
                    # print(cql)
                        try:
                            # self.graph.run(cql, name=name, value=value.replace("'", "\\'"))
                            self.graph.run(cql)
                        except Exception as e:
                            print(e)
                            # print(cql)
...
if __name__=='__main__':
    ...
    # print_data_info("../data/test.json")
